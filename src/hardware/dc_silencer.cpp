                           ...
                           const uint8_t silent_wave_hz)
{
	...
	const double waves = 0.25 + silent_waves;

	// How many sample points exist to generate our waves
	const double steps = waves * sequence_hz / silent_wave_hz;
	rad_dt = waves * 2.0 * PI / steps; // delta radian increment per step
	vol_dt = 1.0 / steps; // delta volume decrement per step
